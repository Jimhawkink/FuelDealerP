  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Link href="/dashboard/customers/new" className="flex items-center gap-2 px-4 py-2 text-white rounded-lg text-sm font-medium" style={{background:"linear-gradient(135deg,#F59E0B,#EA580C)"}}><Plus className="w-4 h-4"/>Add Customer</Link>
      </div>
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        <tbody className="divide-y divide-slate-50">{(data??[]).map((c:any)=>{
          ...
          return <tr key={c.id} className="hover:bg-slate-50">
            <td className="px-4 py-3"><Link href={`/dashboard/customers/${c.id}`} className="font-medium text-amber-600 hover:underline">{c.full_name}</Link></td>
